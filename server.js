...
// ---- CORS ----
// ALLOWED_ORIGINS: قائمة مفصولة بفواصل (،) أو نجمة *
...
app.post("/api/paypal/client-token", async (_req, res) => {
  try {
    ...
    const j = await r.json();
    if (!r.ok || !j?.client_token) return res.status(400).json({ error: "Failed to generate client token", details: j });
...
app.post("/api/paypal/capture", async (req, res) => {
  try {
    ...
    if (status !== "COMPLETED") return res.status(400).json({ error: `Unexpected PayPal status: ${status || "unknown"}`, details: cap });

    const pu    = cap?.purchase_units?.[0] || {};
    const ship  = pu?.shipping?.address || {};
    const name  = pu?.shipping?.name?.full_name || "";
    const payer = cap?.payer || {};
    const [given_name, ...rest] = (name || `${payer?.name?.given_name || ""} ${payer?.name?.surname || ""}`).trim().split(" ");
    const surname = (rest.join(" ").trim()) || payer?.name?.surname || "";
...
app.post("/api/shopify/order-from-paypal", async (req, res) => {
  try {
    const b = req.body || {};
    const errs = [];
    if (!Array.isArray(b.line_items) || b.line_items.length === 0) errs.push("line_items is required");
    const A = b.address || {};
    ...
    if (errs.length) return res.status(400).json({ error: "Invalid payload", details: errs });
    ...
    const draftInput = {
      ...
      billingAddress: { firstName: A.firstName, lastName: A.lastName, address1: A.address1, city: A.city, zip: A.zip, country: A.country, phone: A.phone || null },
      shippingAddress:{ firstName: A.firstName, lastName: A.lastName, address1: A.address1, city: A.city, zip: A.zip, country: A.country, phone: A.phone || null },
      lineItems: b.line_items.map(li => ({ variantId: toVariantGID(li.variant_id), quantity: parseInt(li.quantity, 10) })),
      shippingLine: (b.shipping_price !== "" && b.shipping_price != null) ? { title: (b.shipping_label || "Shipping"), price: String(b.shipping_price) } : null,
    ...
    if (ue1.length) return res.status(400).json({ error: "Shopify user errors", details: ue1 });
    const draftId = d1?.draftOrderCreate?.draftOrder?.id;
    if (!draftId) return res.status(400).json({ error: "Failed to create draft order", details: d1 });
    ...
    if (ue2.length) return res.status(400).json({ error: "Shopify user errors", details: ue2 });
    const orderNode = d2?.draftOrderComplete?.draftOrder?.order;
    if (!orderNode?.id) return res.status(400).json({ error: "Unable to complete draft order", details: d2 });
